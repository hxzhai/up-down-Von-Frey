...
2. **确定起始刺激丝：**  
   ...
   （如选择了 0.07g、0.16g、0.4g、0.6g、1g，则从 0.4g 开始）。
...
4. **终止条件：**  
   当观察到首次阴性与阳性之间的反应转变后，**继续测试 4 次**（共记录 6 个转换后结果）即终止。

5. **输入反应序列：**  
   - 每行输入一组完整反应序列（如 `000101`）；
   - **必须从最中间克重的纤维丝开始记录**，而不是从反应发生变化之后开始记录。
...
1. 点击左上角 「»」按钮 展开侧边栏；
2. 选择测试使用的 **最小克重** 与 **最大克重**；
3. 在文本框中输入一行或多行反应序列（每行一组）；
4. 点击“🚀 开始计算”；
5. 查看计算结果并下载 CSV 文件。

---
###📮如需进一步帮助或修改，请联系维护者 e-mail：zhaihexin1999@163.com
    """)
